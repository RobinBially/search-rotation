...
command = "npx"
args = ["-y", "--allow-git=all", "github:localfoundry/search-rotation#v0.4.7"]
```
...
```sh
claude mcp add search-rotation -- npx -y --allow-git=all github:localfoundry/search-rotation#v0.4.7
```
...
"search-rotation": {
  "type": "local",
  "command": ["npx", "-y", "--allow-git=all", "github:localfoundry/search-rotation#v0.4.7"],
  "codemode": true
}
```
...
```sh
npm install -g ./search-rotation-0.4.7.tgz
```
