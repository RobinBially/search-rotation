...
command = "npx"
args = ["-y", "--allow-git=all", "github:RobinBially/search-rotation#v0.4.0"]
```
...
```sh
claude mcp add search-rotation -- npx -y --allow-git=all github:RobinBially/search-rotation#v0.4.0
```
...
"search-rotation": {
  "type": "local",
  "command": ["npx", "-y", "--allow-git=all", "github:RobinBially/search-rotation#v0.4.0"],
  "codemode": true
}
```
...
```sh
npm install -g ./search-rotation-0.4.0.tgz
```
