...
command = "npx"
args = ["-y", "--allow-git=all", "github:localfoundry/search-rotation#v0.4.8"]
```
...
```sh
claude mcp add search-rotation -- npx -y --allow-git=all github:localfoundry/search-rotation#v0.4.8
```
...
"search-rotation": {
  "type": "local",
  "command": ["npx", "-y", "--allow-git=all", "github:localfoundry/search-rotation#v0.4.8"],
  "codemode": true
}
```
...
```sh
npm install -g ./search-rotation-0.4.8.tgz
```
