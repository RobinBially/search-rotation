...
command = "npx"
args = ["-y", "--allow-git=all", "github:localfoundry/search-rotation#v0.4.1"]
```
...
```sh
claude mcp add search-rotation -- npx -y --allow-git=all github:localfoundry/search-rotation#v0.4.1
```
...
"search-rotation": {
  "type": "local",
  "command": ["npx", "-y", "--allow-git=all", "github:localfoundry/search-rotation#v0.4.1"],
  "codemode": true
}
```
...
If installed locally, use `search-rotation` as the command. A prebuilt archive is also available under [GitHub Releases](https://github.com/localfoundry/search-rotation/releases/latest):

```sh
npm install -g ./search-rotation-0.4.1.tgz
```
