...
/* ---------- Dokumentierte Bugs aus dem static/app.js-Review ---------- */
// Frontend-Bugs lassen sich ohne DOM (jsdom ist nicht installiert) nicht automatisch
// ausführen; sie sind hier als test.skip dokumentiert. Die Suite bleibt grün.

test.skip(
  "FIXED (ungetestbar ohne DOM) — BUG (Rot) static/app.js: enableDrag() hängte bei jedem render() neue Drag-Listener an dasselbe Container-Element → Fix: Listener nur einmal pro Container, Callback wird aktualisiert",
  () => {
    // renderSearch()/renderFetch() rufen enableDrag($("#search-engines")/#("#fetch-engines"), cb)
    // bei JEDER Ausführung auf — mit frischen Closures, also ohne Browser-Dedup.
    // render() läuft bei load(), Sprachwechsel, Refresh-Klick und nach JEDEM putConfig()-Response.
    // Folge: Nach n Re-Renders feuert ein Drop n identische onReorder-Callbacks → n PUT /api/config
    // → n Responses → n weitere render() → Listener-Anzahl verdoppelt sich pro Drop exponentiell
    // (2^n), der Dashboard-Tag flutet irgendwann den Server mit PUTs.
    // Fix: enableDrag nur einmal pro Container registrieren (z. B. beim App-Start) und den
    // Callback über state lesen statt per Closure einzufrieren — oder Listener per AbortSignal
    // vor jedem Re-Render entfernen.
  },
);

test.skip(
  "FIXED (ungetestbar ohne DOM) — BUG (Gelb) static/app.js: parallele putConfig()-Aufrufe auf veraltetem state.config-Snapshot → Fix: queuePut() serialisiert, bodyBuilder läuft zur Ausführungszeit",
  () => {
    // Klickt man schnell hintereinander zwei Schalter an (z. B. A aus, dann B aus, bevor die
    // PUT-A-Response da ist), baut der zweite Handler `engines` aus dem noch alten
    // state.config (A=an) → PUT B enthält A:{enabled:true} → der Server-Merge reaktiviert A.
    // Letzter Schreibvorgang gewinnt, kein Versionierung/Debouncing im Frontend.
    // Fix: putConfig serialisieren (Queue/Promise-Kette) und Payload nach dem letzten
    // Response neu aus state.config bauen, oder optimistisches Merge-Update von state.config
    // sofort beim Klick durchführen statt erst im .then().
  },
);

test.skip(
  "FIXED (ungetestbar ohne DOM) — BUG (Gelb) static/app.js: render() nach putConfig() löscht ungespeicherte Eingaben → Fix: keyinput/extrainput-Werte werden über das Re-Render gerettet",
  () => {
    // Jede putConfig()-Antwort triggert render() → renderSearch() baut ALLE Karten neu auf
    // (innerHTML = ""). Text, der gerade in einem keyinput/extrainput einer ANDEREN Karte
    // getippt, aber noch nicht gespeichert wurde, wird dabei verworfen — z. B. wenn man
    // während des Tippens einen Schalter woanders umlegt. Fix: nur betroffene Karte neu
    // rendern oder Eingabewerte vor dem Re-Render sichern und zurückschreiben.
  },
);
